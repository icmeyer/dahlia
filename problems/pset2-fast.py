...
if find_mix_thermal:
    ...
    plot_k_comparison(mix_ratios, avg_k_fresh, avg_ks, title='Fresh Fuel '+reactor_type)
    
    conc = nat_conc*(1-once_through_mox_conc) + PuAmCm_conc*(once_through_mox_conc)
    conc_over_time, fluxes, ks = main_function(isotopes, conc, reactor_type, years, steps)
    PuAmCm_conc = extract_PuAmCm(isotopes, conc_over_time[:,-1])
    PuAmCm_fasts.append(PuAmCm_conc)
    ...
    plot_k_comparison(mix_ratios, avg_k_fresh, avg_ks, title='Twice Through: '+reactor_type)
    
    conc = nat_conc*(1-twice_through_mox_conc) + PuAmCm_conc*(twice_through_mox_conc)
    conc_over_time, fluxes, ks = main_function(isotopes, conc, reactor_type, years, steps)
    PuAmCm_conc = extract_PuAmCm(isotopes, conc_over_time[:,-1])
    PuAmCm_fasts.append(PuAmCm_conc)
    ...
    avg_ks = run_varying_enrichment(isotopes, iso_names, nat_conc, PuAmCm_conc,
                                    mix_ratios, reactor_type)
    thrice_through_mox_conc = np.interp(avg_k_fresh, avg_ks, mix_ratios)
    print('Pu+ Conc Required Thrice Through: ', thrice_through_mox_conc)
    plot_k_comparison(mix_ratios, avg_k_fresh, avg_ks, title='Thrice Through: '+reactor_type)
    
    conc = nat_conc*(1-thrice_through_mox_conc) + PuAmCm_conc*(thrice_through_mox_conc)
    conc_over_time, fluxes, ks = main_function(isotopes, conc, reactor_type, years, steps)
    PuAmCm_conc = extract_PuAmCm(isotopes, conc_over_time[:,-1])
    PuAmCm_fasts.append(PuAmCm_conc)

    # Make data frame and write PuAmCm Concentrations to file
    PuAmCm_fasts = np.array(PuAmCm_fasts)
    data = np.vstack([np.array(iso_names), PuAmCm_fasts*100])
    data = data.transpose()
    headers = ['Isotope']
    stages = ['5% Enriched', 'Once Burned MOX', 'Twice Burned MOX', 'Thrice Burned MOX']
    ...
    df.at['Weight percent required', 'Twice Burned MOX'] = thrice_through_mox_conc
    csv_path = './'+reactor_type+'_plutonium_vectors_fast_stages.csv'
    df.to_csv(csv_path)
    csv_path =  './'+reactor_type+'_plutonium_vectors_fast_stages_clean.csv'
